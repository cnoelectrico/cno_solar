...
def execute():
    ...
    def on_button_clicked(obj):
        with cen_output:
            ...
            bus_pipeline = cno.pipeline.run2(system_configuration=btn.files['system_configuration'],  ###CNO.PIPELINE.RUN
                                            data=btn.files['df'], 
...
#             df_mes = pd.DataFrame({'GHI': ghi, 'Insolation': insolation,'Temperature': temp})
            ...
            df_hora = btn.files['df']
            cols = list(df_hora.columns)
            df_hora[cols[0]] = df_hora[cols[0]] / 1000 # W to kW
            df_hora = df_hora.loc[df_hora[cols[0]] != 0]

            irrad = df_hora.resample('M').apply(lambda x: x.quantile(0.95))[cols[0]]
            insolation = df_hora[cols[0]].resample('M').sum() # kWh/m2 / month
            temp = df_hora[cols[1]].resample('M').mean()

            df_mes = pd.DataFrame({'GHI': irrad, 'Insolation': insolation,'Temperature': temp})
...
#             months = ['Jan\n2020', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul\n2020', 'Ago', 'Sep', 'Oct', 'Nov', 'Dec']
    ...
    #                                                                       w_enficc.files['enficc_v4'])
            ...
            plt.bar(x, w_cen.files['bus_pipeline']['plant']['system']['energy']['month'].tail(6*1), color='#1580E4',
                    label=plot_label)
